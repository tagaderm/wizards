...
AWizardsGameSession::AWizardsGameSession(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	if (!HasAnyFlags(RF_ClassDefaultObject))
	{
		OnCreateSessionCompleteDelegate = FOnCreateSessionCompleteDelegate::CreateUObject(this, &AWizardsGameSession::OnCreateSessionComplete);
		OnDestroySessionCompleteDelegate = FOnDestroySessionCompleteDelegate::CreateUObject(this, &AWizardsGameSession::OnDestroySessionComplete);

		OnFindSessionsCompleteDelegate = FOnFindSessionsCompleteDelegate::CreateUObject(this, &AWizardsGameSession::OnFindSessionsComplete);
		OnJoinSessionCompleteDelegate = FOnJoinSessionCompleteDelegate::CreateUObject(this, &AWizardsGameSession::OnJoinSessionComplete);

		OnStartSessionCompleteDelegate = FOnStartSessionCompleteDelegate::CreateUObject(this, &AWizardsGameSession::OnStartOnlineGameComplete);
	}
}
...
*/
void AWizardsGameSession::OnStartOnlineGameComplete(FName SessionName, bool bWasSuccessful)
{
	...
	if (bWasSuccessful)
	{
		...
		{
			//AWizardsPlayerController* PC = Cast<AWizardsPlayerController>(*It);
...
void AWizardsGameSession::HandleMatchHasStarted()
{
...
*
*/
void AWizardsGameSession::HandleMatchHasEnded()
{
	// start online game locally and wait for completion
	IOnlineSubsystem* OnlineSub = IOnlineSubsystem::Get();
	if (OnlineSub)
...
bool AWizardsGameSession::IsBusy() const
{
...
EOnlineAsyncTaskState::Type AWizardsGameSession::GetSearchResultStatus(int32& SearchResultIdx, int32& NumSearchResults)
{
	SearchResultIdx = 0;
	NumSearchResults = 0;
...
*/
const TArray<FOnlineSessionSearchResult> & AWizardsGameSession::GetSearchResults() const
{
...
* @param bWasSuccessful true if the async action completed without error, false if there was an error
*/
void AWizardsGameSession::OnCreateSessionComplete(FName SessionName, bool bWasSuccessful)
{
...
* @param bWasSuccessful true if the async action completed without error, false if there was an error
*/
void AWizardsGameSession::OnDestroySessionComplete(FName SessionName, bool bWasSuccessful)
{
...
bool AWizardsGameSession::HostSession(TSharedPtr<const FUniqueNetId> UserId, FName SessionName, const FString& GameType, const FString& MapName, bool bIsLAN, bool bIsPresence, int32 MaxNumPlayers)
{
	...
	if (OnlineSub)
	{
		...
		{
			HostSettings = MakeShareable(new FWizardsOnlineSessionSettings(bIsLAN, bIsPresence, MaxPlayers));
...
void AWizardsGameSession::OnFindSessionsComplete(bool bWasSuccessful)
{
...
void AWizardsGameSession::ResetBestSessionVars()
{
	CurrentSessionParams.BestSessionIdx = -1;
}

void AWizardsGameSession::ChooseBestSession()
{
...
void AWizardsGameSession::StartMatchmaking()
{
	ResetBestSessionVars();
	ContinueMatchmaking();
}

void AWizardsGameSession::ContinueMatchmaking()
{
	ChooseBestSession();
...
void AWizardsGameSession::OnNoMatchesAvailable()
{
	UE_LOG(LogOnlineGame, Verbose, TEXT("Matchmaking complete, no sessions available."));
	SearchSettings = NULL;
}

void AWizardsGameSession::FindSessions(TSharedPtr<const FUniqueNetId> UserId, FName SessionName, bool bIsLAN, bool bIsPresence)
{
	IOnlineSubsystem* OnlineSub = IOnlineSubsystem::Get();
	if (OnlineSub)
	{
		...
		if (Sessions.IsValid() && CurrentSessionParams.UserId.IsValid())
		{
			SearchSettings = MakeShareable(new FWizardsOnlineSearchSettings(bIsLAN, bIsPresence));
...
bool AWizardsGameSession::JoinSession(TSharedPtr<const FUniqueNetId> UserId, FName SessionName, int32 SessionIndexInSearchResults)
{
	bool bResult = false;
...
bool AWizardsGameSession::JoinSession(TSharedPtr<const FUniqueNetId> UserId, FName SessionName, const FOnlineSessionSearchResult& SearchResult)
{
	bool bResult = false;
...
* @param bWasSuccessful true if the async action completed without error, false if there was an error
*/
void AWizardsGameSession::OnJoinSessionComplete(FName SessionName, EOnJoinSessionCompleteResult::Type Result)
{
...
bool AWizardsGameSession::TravelToSession(int32 ControllerId, FName SessionName)
{
	IOnlineSubsystem* OnlineSub = IOnlineSubsystem::Get();
	if (OnlineSub)
	{
